...
function get_emoji(emojip, sname) {
   var Lmoji = { 
    ...
    var sourceName = regionName || cleanName;

    function tokenMatch(source, token) {
        if (!source || !token) { return false }
        var kw = String(token)
        var normalized = source.replace(/[【】\[\]（）()]/g, " ")
        if (/^[A-Za-z]{2,3}$/.test(kw.trim())) {
            var re = new RegExp("(^|[^A-Za-z0-9])" + escapeRegExp(kw.trim()) + "([^A-Za-z0-9]|$)", "i")
            return re.test(normalized)
        }
        return normalized.indexOf(kw) != -1
    }

    function detectByName(source) {
        for (var key in cnt) {
            dd = cnt[key]
            for (i in dd) {
                if (tokenMatch(source, dd[i])) {
                    return key
                }
            }
        }
    ...
    function detectByLeadingFlag(name) {
    ...
    // Prefer the destination/region field before "|" and ignore transit/unlock notes after it.
    var matchFlag = detectByName(sourceName) || detectByLeadingFlag(sname);
    if (!matchFlag && cleanName.indexOf("|") == -1 && cleanName.indexOf("｜") == -1 && cleanName.indexOf("丨") == -1) {
        matchFlag = detectByName(cleanName)
    }
    if (matchFlag) { return [matchFlag + " " + cleanName, matchFlag] }
    return ["🏴‍☠️ " + cleanName, "🏴‍☠️"]
}
